...
describe('constants', () => {
  ...
  describe('API_ENDPOINTS', () => {
    ...
    it('MESSAGESエンドポイントのURLが正しく生成される', async () => {
      ...
      const expectedUrl =
        'https://api.example.com/messages/test-vault/test-user';
    ...
    it('UPDATE_SYNC_STATUSエンドポイントのURLが正しく生成される', async () => {
      ...
      const { API_ENDPOINTS } = await importConstants();
      expect(API_ENDPOINTS.UPDATE_SYNC_STATUS).toBe(
        'https://api.example.com/messages/update-sync-status',
      );
    });
    ...
    it('MESSAGESエンドポイントで空のvaultIdが渡された場合、エラーが投げられる', async () => {
      ...
      const { API_ENDPOINTS } = await importConstants();
      expect(() => API_ENDPOINTS.MESSAGES('', 'user')).toThrow(
        'vaultIdとuserIdは必須パラメータです',
      );
    });
    ...
    it('MESSAGESエンドポイントで空のuserIdが渡された場合、エラーが投げられる', async () => {
      ...
      const { API_ENDPOINTS } = await importConstants();
      expect(() => API_ENDPOINTS.MESSAGES('vault', '')).toThrow(
        'vaultIdとuserIdは必須パラメータです',
      );
    });
    ...
    it('MESSAGESエンドポイントで両方とも空の場合、エラーが投げられる', async () => {
      ...
      const { API_ENDPOINTS } = await importConstants();
      expect(() => API_ENDPOINTS.MESSAGES('', '')).toThrow(
        'vaultIdとuserIdは必須パラメータです',
      );
    });
  });
