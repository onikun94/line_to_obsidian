...
describe('CryptoUtils', () => {
  ...
  describe('Key Export/Import', () => {
    ...
    it('should export and import public key', async () => {
      const exportedPublicKey = await CryptoUtils.exportPublicKey(keyPair.publicKey);
      expect(exportedPublicKey).toContain('-----BEGIN PUBLIC KEY-----');
      expect(exportedPublicKey).toContain('-----END PUBLIC KEY-----');

      const importedPublicKey = await CryptoUtils.importPublicKey(exportedPublicKey);
    ...
    it('should export and import private key', async () => {
      const exportedPrivateKey = await CryptoUtils.exportPrivateKey(keyPair.privateKey);
      expect(exportedPrivateKey).toContain('-----BEGIN PRIVATE KEY-----');
      expect(exportedPrivateKey).toContain('-----END PRIVATE KEY-----');

      const importedPrivateKey = await CryptoUtils.importPrivateKey(exportedPrivateKey);
  ...
  describe('Message Encryption/Decryption', () => {
    ...
    it('should encrypt and decrypt message with AES-GCM', async () => {
      const { encrypted, iv } = await CryptoUtils.encryptMessage(testMessage, aesKey);
  ...
  describe('AES Key Encryption with RSA', () => {
    ...
    it('should encrypt and decrypt AES key with RSA', async () => {
      const encryptedAESKey = await CryptoUtils.encryptAESKey(aesKey, keyPair.publicKey);
      expect(encryptedAESKey).toBeDefined();

      const decryptedAESKey = await CryptoUtils.decryptAESKey(encryptedAESKey, keyPair.privateKey);
  ...
  describe('Key Derivation', () => {
    it('should derive key from device ID', async () => {
      ...
      const derivedKey = await CryptoUtils.deriveKeyFromDeviceId(deviceId, salt);
